      ...
      <main className="mx-auto max-w-3xl px-4 py-12">
        ...
        >
          ...
          <p className="mx-auto mt-2 max-w-md text-sm font-bold leading-6 text-[#777]">
            Your exam overview loads instantly, but full question practice is available with an active DrNote subscription.
          </p>
          <a
            href="/pricing"
...
export default async function QuizPage({ params }) {
  ...
  return (
    <QuizComponent
      ...
      quizName={fileName}
      examName={examName}
      testTaker={user?.user_metadata?.full_name || user?.email || "Test Taker"}
      isSelfExam={false}
    />
  );
}
